...
import { X, Compass, PlusCircle, CheckCircle2, Search } from 'lucide-react';
...
export default function JoinServerModal({
  ...
  // Fetch Public Community Servers
  useEffect(() => {
    async function fetchPublicServers() {
      try {
        const { data } = await supabase.from('servers').select('*').limit(10);
        if (data && data.length > 0) {
          setAvailableServers(data as Server[]);
        } else {
          setAvailableServers([
            { id: 'design-team', name: 'Design Team', owner_id: 'public-owner' },
            { id: 'dev-lounge', name: 'Dev Lounge', owner_id: 'public-owner' },
            { id: 'gaming-hub', name: 'Gaming Hub', owner_id: 'public-owner' },
          ]);
        }
      } catch (err) {
  ...
  const handleJoinById = async (serverIdToJoin: string, serverName: string) => {
    ...
    try {
      // Insert user into server_members table
      ...
      // Fetch first channel of this server
      ...
      setJoinedMsg(`Berhasil bergabung dengan server "${serverName}"!`);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-md p-4 select-none animate-in fade-in duration-200">
      ...
      <div className="w-full max-w-lg bg-[#161619] border border-zinc-800 rounded-3xl p-7 shadow-2xl flex flex-col space-y-6 text-white relative">
        ...
        {/* SECTION 1: JOIN WITH INVITATION / SERVER ID */}
        ...
        <div className="flex items-center my-1">
          ...
          <span className="px-3 text-[10px] uppercase font-bold text-zinc-500 tracking-wider">
            OR EXPLORE PUBLIC COMMUNITY SERVERS
          </span>
          <div className="flex-1 border-t border-zinc-800" />
        </div>

        {/* SECTION 2: DISCOVERABLE PUBLIC SERVERS LIST */}
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-zinc-400">Public Community Server</p>
                </div>
              </div>
